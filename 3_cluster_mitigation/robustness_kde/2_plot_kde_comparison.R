...
KDE_DIR <- file.path(DATA_ROOT, "3_cluster_mitigation", "robustness_kde")
FIG_DIR <- file.path(script_dir, "..", "..", "Figure", "3_cluster_mitigation", "robustness_kde")
...
for (path in c(GSVI_HOTSPOTS, SC_HOTSPOTS)) {
  if (!file.exists(path)) {
    stop(
      "Missing ", path,
      "\nRun: python 3_cluster_mitigation/robustness_kde/1_kde_hotspots.py"
    )
  }
}

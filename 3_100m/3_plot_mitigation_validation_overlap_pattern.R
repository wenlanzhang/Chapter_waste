...
# Brown + white only — soft brown stripes (no dark grey/black)
WHITE <- "#FFFFFF"
LIGHT_GSVI <- "#E5C8A9"
LIGHT_GSC <- "#C9A27F"
STRIPE_GSVI <- "#C9A27F"
...
build_chart <- function(plot_df) {
  ...
  ggplot() +
    ...
    theme(
      ...
      legend.margin = margin(t = 6, r = 0, b = 2, l = 0),
      ...
      plot.margin = margin(t = 4, r = 12, b = 28, l = 8)
    )
}
...
panel <- plot_grid(
  ...
  caption_grob,
  ncol = 1,
  rel_heights = c(0.05, 0.018, 1, 0.10),
  align = "v",
  axis = "l"
)

dir.create(FIG_DIR, recursive = TRUE, showWarnings = FALSE)
out_path <- file.path(FIG_DIR, "Validation_mitigation_self_overlap_pattern.png")
